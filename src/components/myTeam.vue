            ...
            <mu-list v-if="userInfo.team.id">
              ...
              <mu-list-item avatar button :ripple="true">
                <mu-list-item-content>
                  ...
                  <mu-list-item-sub-title><span style="margin-right:10px"> ID：{{userInfo.team.id}}</span> 积分：{{userInfo.team.score}}分</mu-list-item-sub-title>
                </mu-list-item-content>
              ...
              <mu-step>
                ...
                <mu-step-content>
                  ...
                  <mu-dialog :title="userInfo.tel?'修改信息':'请完善信息进行报名'" width="400px" max-width="80%" :esc-press-close="false" :overlay-close="false" :open.sync="openRegister">
                    ...
                    <mu-button slot="actions" color="success" @click="submitEnroll">
              ...
              <mu-step v-if="!userInfo.team.id">
                ...
                <mu-step-content>
                  ...
                  <mu-dialog title="加入队伍" width="400px" max-width="80%" :esc-press-close="false" :overlay-close="false" :open.sync="openJoinTeam">
                    ...
                    <span v-if="teamFound.name" style="color:#000;padding-right:10px"> <strong>{{teamFound.name}}</strong> </span> <span v-if="teamFound.id"> id:{{teamFound.id}} 队长：{{teamFound.leader}}</span>
...
export default {
  data() {
    return {
      ...
      teamfindRules: [
        { validate: val => !!val, message: "请填写查找队伍信息！" },
        {
          validate: val =>
            !/[`~@#$%^&*()+<>:"{},.\/;'·！#￥（——）：；“”‘、，|《。》、【】[\]]/.test(
              val
